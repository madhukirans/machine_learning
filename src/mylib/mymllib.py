...
class MyMlUtil:
    ...
    def logisticClassifer (self, x_train, y_train, x_test):
        #Logistic regression
    ...
    def knnClassifer(self, x_train, y_train, x_test):
        # KNN
        from sklearn.neighbors import KNeighborsClassifier
        knn_classifer = KNeighborsClassifier(n_neighbors=5, metric="minkowski", p=1)
        knn_classifer.fit(x_train, y_train)
        return knn_classifer.predict(x_test)


    def svmClassifer(self, x_train, y_train, x_test):
    ...
    def ksvmClassifer(self, x_train, y_train, x_test):
    ...
    def DTClassifer(self, x_train, y_train, x_test):
        #Decission Tree
    ...
    def RFClassifer(self, x_train, y_train, x_test):
    ...
    def runAllCategoricalModels(self, x_train, y_train, x_test):
         logis_pred = self.logisticClassifer (x_train, y_train, x_test)
         knn_pred = self.knnClassifer(x_train, y_train, x_test)
         svm_pred = self.svmClassifer(x_train, y_train, x_test)
         ksvm_pred = self.ksvmClassifer(x_train, y_train, x_test)
         nb_pred = self.navieBayes(x_train, y_train, x_test)
         dt_pred = self.DTClassifer(x_train, y_train, x_test)
         rf_pred = self.RFClassifer(x_train, y_train, x_test)
         return logis_pred, knn_pred, svm_pred, ksvm_pred, nb_pred, dt_pred, rf_pred

    def writeCsvFiles(self, pred, name, predColumnName, dataframe_col):
    ...
    def backwordElimination(self, x, y):
    ...
    def preprocess(self, dataset, mf_imputer, labelencoder_X) :
    ...
    def printAccuracy_score(self, y_test, logis_pred, knn_pred, svm_pred, ksvm_pred, nb_pred, dt_pred, rf_pred):
    ...
    def printConfusion_matrix(self, y_test, logis_pred, knn_pred, svm_pred, ksvm_pred, nb_pred, dt_pred, rf_pred):
    ...
    def getnullvaluecount(self, dataset):
        print("--------Start dataset-----------")
    ...
    def printValueCount(self, dataset, max_cols=5):
    ...
    def preprocess1(self, dataset, mf_imputer, labelencoder, delete_rows=True):
        ...
        dataset['Dependents'] = dataset['Dependents'].map(
            lambda x: int(x) if str(x).isalnum() and not str(x).isalpha() else self.raise_('Number format exception'))
        ...
        dataset['Credit_History'] = mf_imputer.fit_transform(dataset[['Credit_History']]).ravel()
        self.printValueCount(dataset, 5)
        return dataset
        return dataset